...
### Added
- **CLI output formatting & standard errors**: list/get commands (`users`, `pages list/get`, `databases get`, `blocks children`, `search`) now render a clean, aligned, unicode-width-aware **table by default** instead of dumping raw JSON. A global `--format table|json` flag selects machine output for scripts/pipelines. New `src/cli/output.rs` provides the renderer and Notion field extractors (dependency-free). Errors now print in the conventional `error: <message>: <cause chain>` style to stderr with a non-zero exit code (red `error:` on a TTY, honoring `NO_COLOR`); a global `-v/--verbose` flag prints the full cause chain plus backtrace. Empty result sets print an explicit `No results.` notice rather than looking like a silent failure.
- **Webhook signature verification & payload parsing**: new `blink_md::api::webhooks` surface — `verify_webhook_signature()` validates Notion's `X-Notion-Signature` (`sha256=<hex>`) using HMAC-SHA256 over the raw body with a constant-time comparison; `parse_webhook_payload()` decodes a delivery into a `WebhookPayload` (verification handshake vs. event), with `WebhookEvent`, `WebhookEntity`, and a `WebhookEventType` enum that maps known `page.*`/`database.*`/`comment.*` types and preserves unknown ones as `Other`. Adds `hmac`, `sha2`, `hex`, and `subtle` dependencies (all pure-Rust, no OpenSSL). 7 unit tests.
- **`NotionClient::get_block(id)`**: fetch a single block via `GET /blocks/{id}` (previously only children/append/update/delete existed).
- **`NotionClient::search_all()`**: auto-paginating search helper that follows `next_cursor` until `has_more` is false and returns every result.
- **Enabled the wiremock integration-test suite**: removed the blanket `#[ignore]` so `tests/integration_tests.rs` runs under `cargo test`, fixed three fixtures that had drifted from the models (`List.has_more`, PascalCase `ViewType` tag, `TableConfig.properties`), and added coverage for `get_block` and `search_all` pagination.
- **Markdown YAML frontmatter detection (Phase A)**: new `blink_md::api::markdown_frontmatter::detect_frontmatter()` extracts a `---`-delimited YAML block from the start of a Markdown file and returns the raw YAML text plus the remaining body. Unterminated blocks are treated as plain Markdown. 15 unit tests cover happy paths, multi-line YAML, empty blocks, CRLF inputs, body text containing `---`, and edge cases.
...
- **Adapter architecture proposal**: new `docs/ARCHITECTURE.md` design doc (ADR) proposing a Reader/Writer (file formats) and Source/Sink (live platforms) split to replace the current `Box<dyn Any>` registry, plus a filter/transform stage, capability descriptors for explicit lossy conversion, a `ChangeSet` diff/apply write path, and a frontmatter-based remote-ID addressing model. Includes a platform survey (Notion, Lark/Feishu, GitHub Markdown, Obsidian, AppFlowy, Anytype, Craft) and an incremental M1–M5 migration plan; no code changes yet.
...
- **CI**: `ci.yml`, `coverage.yml`, `cross-platform.yml`, and `audit.yml` now skip documentation-only changes (`**/*.md`, `docs/**`, `LICENSE`, `.gitignore`, `.gitattributes`, `.claude/**`) via `paths-ignore` on both `push` and `pull_request`. Release-tag pushes (`v*`) and the daily audit `schedule` are unaffected.
- Added `.qwen/` to `.gitignore` and package exclusion.

### Fixed
- **Frontmatter/table round-trip hardening** (review follow-ups): `Page::title_from_properties()` now resolves the title column even when it is named something other than `title` (e.g. `Name`), so exported files no longer collapse to `untitled-<id>.md`. Ragged IR table rows are padded to the table width before becoming Notion `TableRow`s (Notion rejects short rows). Body-only tables render with a synthetic empty header instead of promoting the first data row. `PropertyValue::Custom` now serialises to its bare Notion property body instead of being double-wrapped under the property name. Frontmatter `date` properties carry optional `end`/`time_zone` keys so Notion date ranges and zoned dates round-trip. Markdown table blocks set `has_children` when they own row children. Note: GFM cell **alignment** is still lost on the Markdown→Notion→IR path because the Notion table model has no per-column alignment (the render path preserves alignment for IR tables that carry it).
...
- **`search_all`**: now returns `NotionError::PaginationLimitExceeded` instead of silently truncating results if the 1000-page safety cap is hit while the server still reports more results.
