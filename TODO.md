...
- [x] **Phase C — converter**: `MarkdownWithFrontmatterConverter` composes `detect_frontmatter` + `parse_frontmatter_to_properties` + `MarkdownConverter` into a single `FromPlatform`/`ToPlatform` pair that round-trips Markdown+YAML ↔ `UniversalDocument` (with `metadata.properties` populated). 12 unit tests in [`tests/markdown_frontmatter_converter.rs`](tests/markdown_frontmatter_converter.rs).
