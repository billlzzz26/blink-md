...
use crate::ir::{Platform, UniversalDocument, UniversalBlock, table::TableCell, table::TableRow, table::TableRowType};
...
impl FromPlatform for LarkSheetAdapter {
    ...
    fn from_platform(input: Self::Input) -> Result<UniversalDocument, ConverterError> {
        ...
        // Header
        let headers = reader.headers().map_err(|e| ConverterError::InvalidData(e.to_string()))?.clone();
        ...
        for result in reader.records() {
            ...
            let body_row = TableRow {
                ...
                style: None,
                cells: record.iter().map(|c| TableCell {
    ...
    fn to_platform(doc: &UniversalDocument) -> Result<Self::Output, ConverterError> {
        ...
        for block in &doc.blocks {
            if let UniversalBlock::Table { rows, .. } = block {
                for row in rows {
                    let record: Vec<String> = row.cells.iter().map(|cell| {
                        cell.content.iter().map(|inline| {
                            match inline {
                                crate::ir::InlineElement::TextRun { content, .. } => content.clone(),
                                _ => String::new(),
                            }
                        }).collect::<Vec<String>>().join("")
                    }).collect();
                    wtr.write_record(&record).map_err(|e| ConverterError::ConversionFailed(e.to_string()))?;
                }
            }
        }

        let data = String::from_utf8(wtr.into_inner().unwrap()).map_err(|e| ConverterError::ConversionFailed(e.to_string()))?;
        Ok(data)
    }
}
