...
use crate::sync::error::{SyncError, Result};
...
impl PropertyBuilder {
    ...
    pub fn date(mut self, name: &str, value: &str) -> Result<Self> {
        ...
        self.values.insert(name.to_string(), json!({
            "date": { "start": value }
        }));
        Ok(self)
    }
    ...
    pub fn number(mut self, name: &str, value: f64) -> Result<Self> {
        if value.is_nan() {
            ...
            self.values.insert(name.to_string(), json!({ "number": value }));
        }
        Ok(self)
    }
    ...
    pub fn relation(mut self, name: &str, page_ids: Vec<String>) -> Self {
        let ids: Vec<serde_json::Value> = page_ids.into_iter().map(|id| json!({"id": id})).collect();
        self.values.insert(name.to_string(), json!({ "relation": ids }));
        self
    }

    pub fn checkbox(mut self, name: &str, value: bool) -> Self {
        self.values.insert(name.to_string(), json!({ "checkbox": value }));
        self
    }

    pub fn select(mut self, name: &str, option: &str) -> Self {
        self.values.insert(name.to_string(), json!({ "select": { "name": option } }));
        self
    }

    /// Validates and returns the constructed properties.
    pub fn build(self) -> Result<HashMap<String, serde_json::Value>> {
        for (prop_name, _prop_schema) in &self.schema {
