...
fn write_skills_index(entries: &[SkillIndexEntry], output_dir: &str) -> Result<()> {
    ...
    for (cat, heading, subtitle) in [
        ...
        for item in &items {
            // Links are relative to docs/skills.md, pointing at the actual
            // output directory so a non-default `--output-dir` still resolves.
            out.push_str(&format!(
                "| [{}](../{}/{}/SKILL.md) | {} |\n",
                item.name, output_dir, item.name, item.description
            ));
        }
        out.push('\n');
    }
