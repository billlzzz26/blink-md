...
pub async fn handle_generate_skills(args: &[String]) -> Result<()> {
    ...
    // Write skills index
    write_skills_index(&index, output_path)?;

    eprintln!("\nDone. Skills written to {}/", output_dir);
    Ok(())
}

/// Returns the value of `--output-dir` from `args`, defaulting to `"skills"`.
...
/// Writes `content` to `{base}/{name}/SKILL.md`, creating the directory if needed.
...
/// Renders a persona registry entry as a SKILL.md string.
...
/// Renders a recipe registry entry as a SKILL.md string.
...
/// Normalises a description for use in TOML/YAML: replaces `"` with `'` and ensures trailing `.`
...
fn write_skills_index(entries: &[SkillIndexEntry], output_path: &Path) -> Result<()> {
    ...
    // Index lives inside the output directory, so every skill link is a plain
    // relative path — no escaping of the base prefix needed. We still
    // percent-encode the name segment to handle spaces or `()` in names.
    ...
    // Ensure the directory exists before writing — callers may invoke this
    // even when no skill files were generated (e.g. empty or unparseable registry).
    std::fs::create_dir_all(output_path)?;
    let path = output_path.join("INDEX.md");
    std::fs::write(&path, &out)?;
    eprintln!("Skills index written to {}", path.display());
    Ok(())
}
