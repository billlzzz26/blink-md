...
pub async fn handle_generate_skills(args: &[String]) -> Result<()> {
    ...
    if let Ok(registry) = toml::from_str::<PersonaRegistry>(PERSONAS_TOML) {
        eprintln!("Generating skills for {} personas...", registry.personas.len());
...
fn render_persona_skill(persona: &PersonaEntry) -> String {
    let skills_list: Vec<String> = persona.services.iter().map(|s| format!("`{}-mcp`", s)).collect();
    let skills_yaml: Vec<String> = persona.services.iter().map(|s| format!("        - {}-mcp", s)).collect();
...
"#,
        ...
        workflows = persona.workflows.iter().map(|w| format!("- `recipe-{w}`", w = w)).collect::<Vec<_>>().join("\n"),
    ));
...
fn render_recipe_skill(recipe: &RecipeEntry) -> String {
    let skills_yaml: Vec<String> = recipe.services.iter().map(|s| format!("        - {}-mcp", s)).collect();
...
"#,
        ...
        skills = skills_yaml.join("\n"),
        skills_inline = recipe.services.iter().map(|s| format!("`{s}-mcp`")).collect::<Vec<_>>().join(", "),
    ));
...
fn write_skills_index(entries: &[SkillIndexEntry]) -> Result<()> {
    ...
    for (cat, heading, subtitle) in [
        ...
        for item in &items {
            out.push_str(&format!("| [{}](../skills/{}/SKILL.md) | {} |\n", 
