...
pub async fn handle_generate_skills(args: &[String]) -> Result<()> {
    ...
    // Write skills index
    write_skills_index(&index, &output_dir)?;
...
fn write_skills_index(entries: &[SkillIndexEntry], output_dir: &str) -> Result<()> {
    let mut out = String::new();
    out.push_str("# Skills Index\n\n");
    out.push_str("> Auto-generated by `blink-md generate-skills`.\n\n");

    // The index is written to docs/skills.md, so links must reach the output
    // directory where the SKILL.md files actually live. Keep the full path
    // (so nested dirs resolve), normalise Windows separators, and percent-encode
    // spaces. Relative paths are resolved from docs/ via a `../` prefix;
    // absolute paths are linked as-is.
    let normalised = output_dir.replace('\\', "/").replace(' ', "%20");
    let dir_link = if Path::new(output_dir).is_absolute() {
        normalised
    } else {
        format!("../{}", normalised.trim_end_matches('/'))
    };
    ...
    for (cat, heading, subtitle) in [
        ...
        for item in &items {
            out.push_str(&format!(
                "| [{}]({}/{}/SKILL.md) | {} |\n",
                item.name, dir_link, item.name, item.description
            ));
        }
        out.push('\n');
    }
