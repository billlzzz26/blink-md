...
fn write_skills_index(entries: &[SkillIndexEntry], output_dir: &str) -> Result<()> {
    ...
    // The index lives in docs/, so it can only link to the output directory as
    // a sibling. Use its final path component and percent-encode spaces so the
    // Markdown links stay valid regardless of how `--output-dir` was spelled.
    let dir_segment = Path::new(output_dir)
        .file_name()
        .and_then(|s| s.to_str())
    ...
    for (cat, heading, subtitle) in [
        ...
        for item in &items {
            out.push_str(&format!(
                "| [{}](../{}/{}/SKILL.md) | {} |\n",
                item.name, dir_segment, item.name, item.description
            ));
        }
        out.push('\n');
    }
