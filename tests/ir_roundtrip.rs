use blink_md::converter::{FromPlatform, ToPlatform, ConverterError};
...
pub fn perform_roundtrip<F, T>(input: F::Input) -> Result<(UniversalDocument, T::Output), ConverterError>
where
    F: FromPlatform,
    T: ToPlatform,
{
...
mod tests {
    ...
    fn test_markdown_ir_roundtrip_simple() {
        ...
        let (ir, output) = perform_roundtrip::<MarkdownConverter, MarkdownConverter>(input.clone()).unwrap();
    ...
    fn test_cross_platform_roundtrip() {
        ...
        let (ir, notion_req) = perform_roundtrip::<MarkdownConverter, NotionToPlatform>(input).unwrap();
