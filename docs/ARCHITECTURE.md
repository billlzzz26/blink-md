...
1. **Heterogeneous `Input` associated type.** File formats use
   `type Input = String`; Notion uses `type Input = PageWithBlocks`. Because
   ...
   compile error.
2. **No transform stage.** Conversion is strictly `1 platform → IR → 1
   platform`. There is nowhere to put format-agnostic operations (strip
   colors, downgrade unsupported blocks, redact, re-heading, …).
3. **Lossy conversions are implicit.** `block_ir_to_notion` silently returns
   `Err("not implemented")` (pre-table fix) or drops data; the caller can't ask
   "does this target support tables?" up front.
4. **Write path is create-only.** `sync_cmd` always `create_page`s. There is no
   notion of "diff the desired IR against what's live and apply the delta", so
   real sync (update/move/delete) is impossible.
5. **Adapter boilerplate.** Adding a platform means two giant hand-written
   `match` functions (`block_to_ir` / `blocks_to_notion`).
...
```
                         ┌──────────────── filters ───────────────┐
   bytes/String ──Reader──► UniversalDocument ──(transform*)──► UniversalDocument ──Writer──► bytes/String
   (md, html, csv, docx)        ▲   │                                                    (md, html, pdf, …)
                                │   │
   live API  ──Source(async)───┘   └───Sink(async)──►  ChangeSet ──apply──► live API
   (Notion, Lark, GDocs)                                (batchUpdate-style)
```

- **`Reader` / `Writer`** — *synchronous, total functions over bytes.* Uniform
  signatures, **no associated `Input` type**, so the registry holds homogeneous
  trait objects and the `Box<dyn Any>` disappears.
...
- **`Source` / `Sink`** — *async, for live platforms* that fetch/apply over an
  API client. The fetch shape (e.g. `PageWithBlocks`) stays a private detail of
  the adapter, not a public associated type leaking into the registry.

  ```rust
  #[async_trait] pub trait Source { async fn fetch(&self, client: &Client, id: &Ref) -> Result<UniversalDocument, _>; }
...
- Idempotency key on `Sink::apply` so retried syncs don't double-write.
...
The current `FromPlatform`/`ToPlatform` stay working the entire time.

- **M1 — IR contract:** freeze/version the IR; add a doc test snapshot so the
  hub is a stable contract. _(No behaviour change.)_
- **M2 — Reader/Writer traits:** introduce the new traits; provide blanket
  impls bridging existing `FromPlatform<Input=String>`/`ToPlatform<Output=String>`
  so file-format adapters move over with zero rewrites. Drop `Box<dyn Any>` from
  the registry for these.
- **M3 — Filters:** add the transform stage + 1–2 real filters (e.g.
  "strip default colours", "downgrade unsupported block").
...
- **M5 — ChangeSet write path:** `diff(current, desired) → ChangeSet`; teach
  `sync` to reconcile (update/move/delete), not just create.
...
`batchUpdate`-style ChangeSets** for the write path, and **Stripe conventions**
for error/idempotency/versioning. Land it via M1→M5 so `main` stays green and
